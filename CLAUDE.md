...
- **Columnas K y L en `Creados` son fórmulas Excel en español, no valores pre-calculados** — `=EXTRAE(D{n};1;2)` y `=SI.CONJUNTO(K{n}="19";...)`. Razón: cliente lo pidió explícitamente; mantener fórmulas permite que un usuario que edite D manualmente vea K y L recalcular en Excel. openpyxl serializa strings que empiezan con `=` como fórmulas (`data_type='f'` en el XLSX) y el XML interno preserva los nombres en español tal cual — Excel-ES los ejecuta directamente. Como openpyxl NO evalúa fórmulas, los tests sólo pueden verificar el string de la fórmula — la lógica de clasificación se valida visualmente al abrir en Excel.
- **Fórmulas en español, no inglés** — Excel-ES no interpreta correctamente `=MID(...,...,...)` o `=IFS(...)` con `,` como separador (los `,` se interpretan como decimales en locale-ES). Hay que escribir `EXTRAE`/`SI.CONJUNTO`/`VERDADERO` con `;` como separador. openpyxl no traduce: lo que se escribe es lo que queda en el .xlsx.
