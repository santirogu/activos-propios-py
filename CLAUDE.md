...
- **Sociedades (multiselect)**: **checkboxes** (`tk.Checkbutton` + `tk.BooleanVar` por sociedad) en grilla de 2 columnas — más amigable que un Listbox con barras de selección. Opciones: `TRAN, ISA, ITCH, CEYA, CABA, RPAE, CTMP, REPD, ISAP, XM`. Debajo, caption gris *"Selecciona 1 o más sociedades para generar el Reporte SOX"*. El usuario marca **1 o varias**; se genera un reporte por cada una (no consolidado). El helper `_sociedades_seleccionadas()` (expuesto en el frame) devuelve las marcadas preservando el orden de `VALID_SOCIEDADES`; los `BooleanVar` se exponen como `frame_sox.soc_vars` (dict).
...
- **Saltos de línea internos en una celda corromperían el `.txt`**: LSMW lee por posición de columna, así que `export_sheet_to_tsv` neutraliza (`_sanitizar_celda_tsv`) los `\n`/`\r`/`\t` embebidos reemplazándolos por espacio. Sin esto, una celda como ZZPOSNR con `'RI-POAS1-7.1.01\n '` parte el registro en varias líneas físicas y desplaza todas las columnas siguientes → SAP recibe campos corridos/vacíos (ej. el KOSTL sale vacío y hay que llenarlo a mano). Cada fila del Excel = exactamente 1 línea física de N campos.
...
## 8. Pruebas — 383 tests con `unittest`
